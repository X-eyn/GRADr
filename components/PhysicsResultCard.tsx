...
const PhysicsResultCard: React.FC<PhysicsResultCardProps> = ({ result, onReset }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="bg-gradient-to-br from-red-900/20 to-orange-900/20 rounded-3xl p-6 border border-red-500/20 backdrop-blur-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          ...
          <div className="text-left">
            ...
            <p className="text-red-300/70 text-sm mt-1 font-coolvetica">Assessment Results</p>
          </div>
            ...
            {result.stats && (
              <>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  📝 {result.stats.questionsCorrect}/{result.stats.questionsTotal} Problems
                </div>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  🧮 Calculations: {result.stats.grammarLevel}
                </div>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  ⚛️ Concepts: {result.stats.conceptsLevel}
                </div>
              </>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="sticky top-24">
            ...
            {result.uploadedImage ? (
              <div className="bg-neutral-900/50 rounded-2xl border border-red-500/20 overflow-hidden">
                <img 
            ...
            {result.questions && result.questions.length > 0 && (
              <div className="mt-6 bg-neutral-900/30 rounded-2xl border border-red-500/20 p-4">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {result.actionItems && result.actionItems.length > 0 && (
            <div className="bg-gradient-to-br from-red-900/20 to-orange-900/20 rounded-2xl border border-red-500/20 p-6">
              <h3 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="space-y-3">
                {result.actionItems.map((item, index) => (
                  <label key={index} className="flex items-start gap-3 group cursor-pointer">
                    <input 
                      type="checkbox" 
                      className="mt-1 w-4 h-4 rounded border-white/20 bg-white/5 text-red-500 focus:ring-2 focus:ring-red-500 focus:ring-offset-0 cursor-pointer"
                    />
...
// Physics Question Card Component
const PhysicsQuestionCard: React.FC<{ question: QuestionFeedback; isSelected: boolean }> = ({ question, isSelected }) => {
  const statusConfig = {
    correct: {
      icon: '✅',
    ...
    >
      ...
      <div className="mb-4 bg-neutral-800/50 rounded-xl p-4 border border-white/5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-neutral-400 uppercase tracking-wide">Your Solution</span>
        </div>
      ...
      {question.status !== 'correct' && question.correctAnswer && (
        <div className="mb-4 bg-green-900/10 rounded-xl p-4 border border-green-500/20">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-green-400 uppercase tracking-wide">Correct Solution</span>
          </div>
      ...
      {question.insight && (
        <div className="bg-orange-900/10 rounded-xl p-4 border border-orange-500/20">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <span className="text-xs font-medium text-orange-400 uppercase tracking-wide block mb-1">Physics Insight</span>
...
export default PhysicsResultCard;
