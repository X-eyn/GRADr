...
const ChemistryResultCard: React.FC<ChemistryResultCardProps> = ({ result, onReset }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="bg-gradient-to-br from-green-900/20 to-lime-900/20 rounded-3xl p-6 border border-green-500/20 backdrop-blur-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
          ...
          <div className="text-left">
            ...
            <p className="text-green-300/70 text-sm mt-1 font-coolvetica">Assessment Results</p>
          </div>
            ...
            {result.stats && (
              <>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  📝 {result.stats.questionsCorrect}/{result.stats.questionsTotal} Questions
                </div>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  ⚗️ Equations: {result.stats.grammarLevel}
                </div>
                <div className="px-4 py-1.5 rounded-full bg-neutral-800/30 border border-white/5 text-xs text-neutral-300 w-fit">
                  🧪 Concepts: {result.stats.conceptsLevel}
                </div>
              </>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="sticky top-24">
            ...
            {result.uploadedImage ? (
              <div className="bg-neutral-900/50 rounded-2xl border border-green-500/20 overflow-hidden">
                <img 
            ...
            {result.questions && result.questions.length > 0 && (
              <div className="mt-6 bg-neutral-900/30 rounded-2xl border border-green-500/20 p-4">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {result.actionItems && result.actionItems.length > 0 && (
            <div className="bg-gradient-to-br from-green-900/20 to-lime-900/20 rounded-2xl border border-green-500/20 p-6">
              <h3 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="space-y-3">
                {result.actionItems.map((item, index) => (
                  <label key={index} className="flex items-start gap-3 group cursor-pointer">
                    <input 
                      type="checkbox" 
                      className="mt-1 w-4 h-4 rounded border-white/20 bg-white/5 text-green-500 focus:ring-2 focus:ring-green-500 focus:ring-offset-0 cursor-pointer"
                    />
          ...
          {result.questions && result.questions.length > 0 ? (
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-white flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-lime-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// Chemistry Question Card Component
const ChemistryQuestionCard: React.FC<{ question: QuestionFeedback; isSelected: boolean }> = ({ question, isSelected }) => {
  const statusConfig = {
    correct: {
      icon: '✅',
  ...
  return (
    <div 
      ...
      className={`bg-neutral-900/50 rounded-2xl border ${config.borderColor} ${config.bgColor} p-6 transition-all ${
        isSelected ? 'ring-2 ring-green-500 scale-[1.02]' : ''
      }`}
    >
      {/* Question Header */}
      ...
      <div className="mb-4 bg-neutral-800/50 rounded-xl p-4 border border-white/5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-neutral-400 uppercase tracking-wide">Your Answer</span>
        </div>
        <p className="text-neutral-300 text-sm font-mono leading-relaxed whitespace-pre-wrap">
      ...
      {question.status !== 'correct' && question.correctAnswer && (
        <div className="mb-4 bg-green-900/10 rounded-xl p-4 border border-green-500/20">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-green-400 uppercase tracking-wide">Correct Answer</span>
          </div>
          <p className="text-green-300 text-sm font-mono leading-relaxed whitespace-pre-wrap">
      ...
      {/* AI Insight */}
      {question.insight && (
        <div className="bg-lime-900/10 rounded-xl p-4 border border-lime-500/20">
          <div className="flex items-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-lime-400 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div className="flex-1">
              <span className="text-xs font-medium text-lime-400 uppercase tracking-wide block mb-1">Chemistry Insight</span>
...
export default ChemistryResultCard;
